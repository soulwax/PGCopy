...
Do not re-add completed items: partial-failure summary, dry-run row counts, destination readiness checks, schema copy via pg_dump, drop-schema repair flow, SSH tunneling, `~/.ssh/config` auto-population, desktop preflight checks, desktop `Get pg tools`, desktop history, desktop log export, desktop log severity coloring, expanded URL diagnostics, published desktop smoke checks, elapsed completion summaries, `--schema-only` / `--data-only`, and whole-server all-databases overwrite (`--all-databases` / `--exclude-database` CLI flags with matching desktop UI) are all done.
