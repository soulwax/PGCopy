...
Do not re-add completed items: partial-failure summary, dry-run row counts, destination readiness checks, schema copy via pg_dump, drop-schema repair flow, SSH tunneling, `~/.ssh/config` auto-population, desktop preflight checks, desktop `Get pg tools`, desktop history, desktop log export, desktop log severity coloring, expanded URL diagnostics, published desktop smoke checks, elapsed completion summaries, `--schema-only` / `--data-only`, whole-server all-databases overwrite (`--all-databases` / `--exclude-database` CLI flags with matching desktop UI), database-less URLs for `--all-databases`, SSL required by default with per-endpoint opt-out, and a per-user NSIS installer for the desktop app are all done.
