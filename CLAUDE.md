...
- `pg_dump` / `psql` resolve in interactive PowerShell but may not resolve in non-interactive shells. `SchemaCreator` performs an explicit PATH check and surfaces a clear error.
...
- **No stored credentials.** Connection strings stay in memory; never written to disk.
...
Per `AGENTS.md`, do not introduce: an ORM, non-PostgreSQL engines, a local web UI (the prototype was removed deliberately — the desktop `.exe` is the no-terminal path), background services, stored credentials, cloud-provider-specific branches, a general ETL framework, or upsert/conflict-resolution modes.
