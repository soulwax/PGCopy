...
Cloning a Postgres database currently requires knowing pg_dump flags, psql invocation, and managing the pipe yourself. PGCopy makes it a guided, visual, two-prompt terminal operation — you give it an origin URL and a destination URL and it handles everything.

## Decisions

| Question | Decision | Rationale |
|---|---|---|
| Invocation model | Interactive TUI | "Laughably easy" — no flags to remember |
...
| TUI library | Spectre.Console | De-facto .NET TUI standard |
...
├── Program.cs              TUI only — prompts + live table rendering
...
5. LIVE TABLE    Spectre.Console LiveTable: Table | Rows | Status (updates in-place)
